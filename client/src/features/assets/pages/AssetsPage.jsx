...
import {
  ...
  Box, RotateCcw, AlertTriangle, CalendarClock, History, Mouse, Headphones, Pencil, Trash2,
} from 'lucide-react';
...
import {
  ...
  useCreateAssetMutation, useUpdateAssetMutation, useDeleteAssetMutation, useAssignAssetMutation,
...
export default function AssetsPage() {
  ...
  const confirmDelete=async()=>{try{await deleteAsset(deleteTarget._id).unwrap();toast.success('Asset deleted.');if(selected===deleteTarget._id)setSelected(null);setDeleteTarget(null);}catch(error){toast.error(error?.data?.error?.message||'Unable to delete asset.');}};
  const sendToRepair=async body=>{try{await addMaintenance({id:repairTarget._id,...body}).unwrap();toast.success('Asset moved to Under Repair.');setRepairTarget(null);}catch(error){toast.error(error?.data?.error?.message||'Unable to send asset for repair.');}};
  ...
  return <div className="space-y-6 p-4 sm:p-6">
    ...
    {isLoading?<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{[1,2,3,4,5,6].map(x=><Skeleton key={x} className="h-44"/>)}</div>:assets.length?<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{assets.map(asset=><article key={asset._id} role="button" tabIndex={0} onClick={()=>setSelected(asset._id)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();setSelected(asset._id);}}} className="cursor-pointer rounded-2xl border border-border bg-card p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 gap-3"><div className="rounded-xl bg-primary/10 p-2.5"><Laptop className="h-5 w-5 text-primary"/></div><div className="min-w-0"><p className="truncate font-semibold">{asset.name}</p><p className="text-xs text-muted-foreground">{asset.assetCode} · {asset.category}</p></div></div><Badge variant={STATUS_VARIANTS[asset.status]}>{STATUS_LABELS[asset.status]}</Badge></div><div className="mt-4 grid grid-cols-2 gap-2 border-t border-border pt-3 text-sm"><p><span className="text-xs text-muted-foreground">Brand / Model</span><br/><b>{[asset.brand,asset.model].filter(Boolean).join(' ')||'—'}</b></p><p><span className="text-xs text-muted-foreground">Assigned To</span><br/><b>{asset.assignedEmployeeId?.fullName||'Available'}</b></p><p><span className="text-xs text-muted-foreground">Serial</span><br/><b>{asset.serialNumber||'—'}</b></p><p><span className="text-xs text-muted-foreground">Warranty</span><br/><b>{formatDate(asset.warrantyExpiryDate)}</b></p></div>{canManage&&<div className="mt-4 flex flex-wrap justify-end gap-2 border-t border-border pt-3">{asset.status!=='under_repair'&&<Button type="button" size="sm" variant="outline" onClick={event=>{event.stopPropagation();setRepairTarget(asset);}}><Wrench className="h-4 w-4"/> Send to Repair</Button>}<Button type="button" size="sm" variant="outline" onClick={event=>{event.stopPropagation();setAssetForm(asset);}}><Pencil className="h-4 w-4"/> Edit</Button><Button type="button" size="sm" variant="danger" onClick={event=>{event.stopPropagation();setDeleteTarget(asset);}}><Trash2 className="h-4 w-4"/> Delete</Button></div>}</article>)}</div>:<div className="rounded-2xl border border-border bg-card p-16 text-center"><Package className="mx-auto mb-3 h-10 w-10 text-muted-foreground"/><p className="font-medium">No assets found</p><p className="text-sm text-muted-foreground">{canManage?'Add your first company asset.':'No assets are currently assigned to you.'}</p></div>}
    </div>
    ...
    <Modal isOpen={Boolean(repairTarget)} onClose={()=>setRepairTarget(null)} title={repairTarget?`Send ${repairTarget.name} to Repair`:'Send to Repair'} size="md">{repairTarget&&<ActionForm type="maintenance" employees={employees} onSubmit={sendToRepair} onClose={()=>setRepairTarget(null)} loading={maintaining}/>}</Modal>
    <ConfirmDialog isOpen={Boolean(deleteTarget)} onCancel={()=>setDeleteTarget(null)} onConfirm={confirmDelete} isLoading={deleting} title="Delete asset?" message={deleteTarget?`${deleteTarget.name} will be permanently removed. Assigned or under-repair assets must be returned first.`:''} confirmLabel="Delete Asset"/>
  </div>;
}
